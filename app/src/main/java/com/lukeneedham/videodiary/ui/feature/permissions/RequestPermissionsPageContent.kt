...
fun RequestPermissionsPageContent(
    ...
    val isSettledGranted = settledPermission != null && settledPermission.permission in acquiredPermissions

    LaunchedEffect(settledPage, isSettledGranted) {
        if (!isSettledGranted) return@LaunchedEffect
